...
namespace Series.Core
{
	...
	[AddComponentMenu("Series/Platformer/Player/Player")]
	public class Player : Entity<Player>
	{
        [Header("Combo Settings")]
        [Tooltip("Maximum time window to input next combo attack")]
        [SerializeField] private float comboWindow = 0.5f;

        [Tooltip("Time before reverting to normal idle state")]
        [SerializeField] private float idleTransitionTime = 2f;

		[Tooltip("Time Between Inputs")]
        ...
        /// <summary>
        /// Returns the Power instance.
        /// </summary>
        public Power power { get;  set; }

        /// <summary>
        /// Returns the Stamina instance.
        /// </summary>
        ...
        /// <summary>
        /// Returns true if the Player Stamina is not empty.
        /// </summary>
        public virtual bool isStamina => !stamina.isEmpty;

        /// <summary>
        /// Returns true if the Player power is not empty.
        /// </summary>
        public virtual bool isPower => !power.isEmpty;
		...
		private PlayerEquipmentSystem swordSystem;
        ...
        protected virtual void InitializePower() => power = GetComponent<Power>();
        protected virtual void InitializeStamina() => stamina = GetComponent<Stamina>();
		...
		public virtual void Attack()
		{
            ...
            // Check if we should exit combat state
            if (isInCombo && Time.time - lastAttackTime > idleTransitionTime)
            {
        ...
        /// <summary>
        /// Attempts to perform an attack based on current combo state
        /// </summary>
        ...
        /// <summary>
        /// Initiates the combo sequence
        /// </summary>
        private void StartCombo()
        {
            isInCombo = true;
            currentComboIndex = 0;
            states.Change<AttackOnePlayerState>();
            ...
            comboResetCoroutine = StartCoroutine(ComboWindowRoutine());
        }
        ...
        /// <summary>
        /// Queues the next attack in the combo sequence if within valid window
        /// </summary>
        private void QueueNextComboAttack()
        {
			if (currentComboIndex >= 3) { currentComboIndex = 0; return; } // Max 4 attacks (0-3)

            // Queue next attack
            currentComboIndex++;

            // Switch to appropriate attack state
            switch (currentComboIndex)
            {
                case 1:
                    states.Change<AttackTwoPlayerState>();
                    break;
                case 2:
                    states.Change<AttackThreePlayerState>();
                    break;
                case 3:
                    states.Change<AttackHPlayerState>();
                    break;
            }
        ...
        /// <summary>
        /// Coroutine to handle the combo window timing
        /// </summary>
        private IEnumerator ComboWindowRoutine()
        {
            canReceiveInput = false;

            // Wait for a brief moment to prevent immediate input
            ...
            // If no new input received, reset combo
            if (canReceiveInput)
            {
                ResetCombo();
            }
        ...
        /// <summary>
        /// Resets the combo state to allow a new combo to start
        /// </summary>
        private void ResetCombo()
        {
            ...
            {
                idleTransitionCoroutine = StartCoroutine(IdleTransitionRoutine());
            }
        }

        /// <summary>
        /// Coroutine to handle transition back to idle state
        /// </summary>
        private IEnumerator IdleTransitionRoutine()
        {
		...
		protected override void Awake()
		{
			...
			swordSystem = GetComponent<PlayerEquipmentSystem>();
		}
